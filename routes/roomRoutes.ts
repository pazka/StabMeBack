...
import {addPlayerToRoom, kickPlayerOfRoom, createRoom, getAllRooms, getRoom, removeRoom} from "../Controllers/roomController";
...
router.post('/create',
    ...
    check('dropInterval').default(getConfig("DefaultValues.APDropInterval")).isInt({
        min: 1,
        max: 7 * 24 * 60
    }).withMessage('Must be 1 < x < 7*24*60'),
    check('dropAmount').default(getConfig("DefaultValues.APDropAmount")).isInt({
        min: 1,
        max: 1000
    }).withMessage('Must be 1 < x < 1000'),
    check('startAP').default(getConfig("DefaultValues.startAP")).isInt({
        min: 0,
        max: 1000
    }).withMessage('Must be 0 < x < 1000'),
    check('startHP').default(getConfig("DefaultValues.startHP")).isInt({
        min: 1,
        max: 1000
    }).withMessage('Must be 1 < x < 1000'),
    check('startRange').default(getConfig("DefaultValues.startRange")).isInt({
        min: 0,
        max: 1000
    }).withMessage('Must be 0 < x < 1000'),
    check('maxPlayers').default(getConfig("DefaultValues.maxPlayers")).isInt({
        min: 2,
        max: 20
    }).withMessage('Must be 2 < x < 20'),
    check('roomSize').default(getConfig("DefaultValues.roomSize")).isInt({
        min: 2,
        max: 1000
    }).withMessage('Must be 2 < x < 1000'),
